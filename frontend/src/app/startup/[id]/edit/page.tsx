...
export default function EditStartupPage() {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const body = {
      ...
      founded_date: formValues.founded_date || null,
      corporate_number: corpDigits || null,
      business_registration_number: brnDigits || null,
      ceo_name: formValues.ceo_name || null,
      current_employees: toInt("current_employees"),
      location: formValues.location || null,
      ksic_code: selectedKsic?.code || null,
      industry: selectedKsic?.name || null,
      main_product: formValues.main_product || null,
      stock_market: formValues.stock_market || null,
    ...
    try {
      await api.patch(`/startups/${startup.id}`, body);
  ...
  }, [startup, router, corpNum, brn, selectedKsic, formValues]);

  if (pageLoading) return <div className="p-8 text-center text-slate-400">로딩 중...</div>;
  if (!startup) return <div className="p-8 text-center text-slate-400">스타트업을 찾을 수 없습니다.</div>;

  return (
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => router.push(`/startup/${startup.id}`)}>
            <ArrowLeft size={18} />
          </Button>
          <h2 className="text-lg font-bold text-slate-800">스타트업 수정</h2>
        </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <ControlledField label="기업(고객)명" value={formValues.company_name ?? ""} onChange={(v) => updateField("company_name", v)} required />
          <ControlledField label="설립일자" value={formValues.founded_date ?? ""} onChange={(v) => updateField("founded_date", v)} type="date" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <ControlledField label="대표자" value={formValues.ceo_name ?? ""} onChange={(v) => updateField("ceo_name", v)} />
          <ControlledField label="임직원수" value={formValues.current_employees ?? ""} onChange={(v) => updateField("current_employees", v)} type="number" />
        </div>
        <ControlledField label="소재지" value={formValues.location ?? ""} onChange={(v) => updateField("location", v)} />
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <ControlledField label="주요사업(제품)" value={formValues.main_product ?? ""} onChange={(v) => updateField("main_product", v)} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <ControlledSelect label="상장시장" value={formValues.stock_market ?? ""} onChange={(v) => updateField("stock_market", v)} options={STOCK_MARKETS} />
          <ControlledField label="상장일자" value={formValues.listing_date ?? ""} onChange={(v) => updateField("listing_date", v)} type="date" />
        </div>

        {/* 재무정보 */}
        <SectionTitle>재무정보</SectionTitle>
        <div className="grid grid-cols-2 gap-4">
          <MoneyField label="자산총액" value={formValues.total_assets ?? ""} onChange={(v) => updateField("total_assets", v)} />
          <MoneyField label="자본금" value={formValues.capital ?? ""} onChange={(v) => updateField("capital", v)} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <MoneyField label="매출액" value={formValues.current_revenue ?? ""} onChange={(v) => updateField("current_revenue", v)} />
          <MoneyField label="영업이익" value={formValues.operating_profit ?? ""} onChange={(v) => updateField("operating_profit", v)} />
        </div>

        {/* 연구개발 */}
        <SectionTitle>연구개발</SectionTitle>
        <div className="grid grid-cols-2 gap-4">
          <ControlledSelect label="기술연구소" value={formValues.has_research_lab ?? ""} onChange={(v) => updateField("has_research_lab", v)} options={RESEARCH_LAB_OPTIONS} />
          <ControlledField label="연구인력" value={formValues.research_staff_count ?? ""} onChange={(v) => updateField("research_staff_count", v)} type="number" />
        </div>

        {/* 연락처 정보 */}
        <SectionTitle>연락처 정보</SectionTitle>
        <div className="grid grid-cols-2 gap-4">
          <ControlledField label="도시" value={formValues.city ?? ""} onChange={(v) => updateField("city", v)} />
          <ControlledField label="웹사이트" value={formValues.website ?? ""} onChange={(v) => updateField("website", v)} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <ControlledField label="실무담당" value={formValues.contact_person ?? ""} onChange={(v) => updateField("contact_person", v)} />
          <ControlledField label="연락처" value={formValues.contact_phone ?? ""} onChange={(v) => updateField("contact_phone", v)} />
        </div>
        <ControlledField label="이메일" value={formValues.contact_email ?? ""} onChange={(v) => updateField("contact_email", v)} type="email" />
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Button type="button" variant="outline" onClick={() => router.push(`/startup/${startup.id}`)}>취소</Button>
...
function ControlledField({ label, value, onChange, type = "text", required, placeholder }: {
...
function ControlledSelect({ label, value, onChange, options }: {
...
function MoneyField({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  const millions = value.trim() ? (Number(value.replace(/,/g, "")) / 1000000).toFixed(0) : null;
  return (
    <div>
      <label className="block text-sm font-medium text-slate-600 mb-1">{label}(원)</label>
      <input type="number" value={value} onChange={(e) => onChange(e.target.value)}
        className="w-full h-9 px-3 border border-slate-300 rounded-md text-sm outline-none focus:border-blue-500" />
      {millions !== null && <p className="text-xs text-blue-600 mt-1">= {Number(millions).toLocaleString()} 백만원</p>}
    </div>
  );
}
